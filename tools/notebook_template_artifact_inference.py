...
TEMPLATE = {
    **{k: _E2E[k] for k in ("package", "repo_name", "pipeline_class", "weights_key", "modules", "entry_module", "model_load", "runtime_imports")},
    ...
    "title": "TabDPT Classifier — DIMER artifact inference tutorial (standalone)",
    "badges": [
        badge
        if badge[0] != "Open In Colab"
            ...
            badge[0],
            badge[1],
            f"https://colab.research.google.com/github/kurtvalcorza/{REPO}/blob/main/tutorials/tabdpt_classifier_artifact_inference_colab.ipynb",
        )
        for badge in BADGES
    ],
    "capability": "serving-state reconstruction from an externally produced DIMER artifact (`artifact.json` + `training_context.parquet`) and class-probability inference on genuinely new rows with the pinned `Layer6/TabDPT` v1.2 checkpoint",
    "intro": (
        ...
        "provenance, restores the fitted class-label mapping and preprocessing/support state, accepts genuinely new "
        "unlabelled data, predicts class labels with class probabilities, and exports results. No gradient fine-tuning "
    ...
    "learning_objectives": (
        ...
        "unlabelled rows into an input manifest, predict with class probabilities under an explicit `argmax` rule, "
    ...
    "exclusions": (
        "artifact creation, in-notebook support fitting, regression, gradient fine-tuning, calibrated probabilities, "
        "or any quality claim: without labelled rows nothing is measured, and the exported probabilities are "
        "uncalibrated ensemble outputs with no shipped threshold."
    ),
    "prerequisites": [
    ...
    "cells": [
        {
            "md": (
                ...
                "`validate_artifact_bundle` runs **before** reconstruction and checks format/task, the class-label "
                "mapping, the support table's path/size/SHA-256, fitted-preprocessing consistency, and the manifest's "
                ...
                "provenance a consumer needs: format, version, base model, class labels, feature schema and context digest."
            ),
            "code": (
                "import json\n"
                "import os\n\n"
                ...
                "class_labels = list(manifest['preprocessing']['classLabels'])\n"
                ...
                "print({{'classLabels': class_labels, 'featureCount': len(expected), 'numericColumns': numeric_columns, 'categoricalColumns': categorical_columns, 'contextBytes': context_path.stat().st_size, 'contextSha256': manifest['trainingContext']['sha256']}})"
            ),
        },
        {
            "md": (
                "## 5. Reconstruct the serving state with fitted preprocessing restored\n\n"
                "`load_verified_artifact` re-validates the artifact, restores the serialised feature encoder and "
                "class-label mapping, the fitted upstream mean-imputation and standardisation state and any saved PCA "
                ...
                "reconditioning path or a class-label disagreement. It also surfaces the model feature ceiling and "
            ...
            "code": (
                ...
                "if serving.class_labels_ != class_labels:\n"
                "    raise RuntimeError('Reconstructed class labels disagree with the validated manifest.')\n"
        ...
        },
        {
            "md": (
                ...
                "pre-existing `prediction`/`proba_*` columns. For CSV, categorical columns are read explicitly as strings "
                ...
                "stage for inference tables: it applies exactly the schema check `predict_proba` applies and returns an "
            ...
            "code": (
                ...
                "reserved = [serving.target_column, 'prediction', *[f'proba_{{label}}' for label in serving.class_labels_]]\n"
                "present_reserved = [c for c in reserved if c in new_data]\n"
                "if present_reserved:\n"
                "    raise ValueError(f'Remove target/prediction/proba columns before inference: {{present_reserved}}')\n"
                "print({{'ceilings': {{'MIN_CLASSES': MIN_CLASSES, 'DECISION_RULE': DECISION_RULE, 'modelFeatureCeiling': model_feature_ceiling}}}})\n"
        ...
        },
        {
            "md": (
                ...
                "`predict_proba()` returns one column per fitted class **in the artifact's stored (sorted) class "
                "order** and `prediction` is the `argmax` label — the pipeline ships no probability threshold and the "
                "probabilities are uncalibrated ensemble outputs; the caller owns downstream calibration. `row_id` maps "
                ...
                "prediction CSV keeps the `proba_<class>` columns in class order, and the result JSON records the "
                "externally supplied artifact identity, the immutable model contract, the class labels, the restored "
            ...
            "code": (
                ...
                "proba = serving.predict_proba(new_data, **kw)\n"
                "pred = serving.predict(new_data, **kw)\n"
                "results = pd.DataFrame({{'row_id': new_data.index.to_numpy(), 'prediction': pred.to_numpy()}})\n"
                "for label in serving.class_labels_:\n"
                "    results[f'proba_{{label}}'] = proba[label].to_numpy()\n"
                "results.to_csv('outputs/{stem}_predictions.csv', index=False)\n"
                "report = evaluation_report(None, n_holdout=0, class_labels=serving.class_labels_, sample_kind='BYOD')\n"
                ...
                "    'artifact': {{'source': artifact_source, 'format': manifest['format'], 'formatVersion': manifest.get('formatVersion'), 'baseModel': manifest['baseModel'], 'classLabels': serving.class_labels_, 'trainingContextSha256': manifest['trainingContext']['sha256'], 'contextRows': context_rows}},\n"
                ...
                "    'inference': {{**kw, 'decisionRule': DECISION_RULE + ' over uncalibrated class probabilities', 'threshold': None, 'effectiveSupportRowsAtMost': min(context_rows, kw['context_size'])}},\n"
    ...
    "closing": (
        ...
        "carried package's pinned model contract, that the class-label mapping and fitted preprocessing were restored "
        "without refitting, that the support context was reconstructed, and that schema-compatible new records were "
        "scored with explicit capacity/context behaviour under an explicit `argmax` decision rule — without the "
        ...
        "no labels exist here, and the exported probabilities must not be thresholded without the caller's own "
        "calibration data. Never bypass a failed manifest, digest, class-label, preprocessing-state or schema check; "
        ...
        "policy in the manifest; compare predictions at `n_ensembles` 1 versus 4; hand the exported predictions and a "
        "labelled copy of the same rows to `evaluate` in the E2E tutorial to obtain a `sample-sanity` report.\n\n"
        "## References\n\n"
        ...
        f"- E2E companion (produces the artifact): https://github.com/kurtvalcorza/{REPO}/blob/main/tutorials/tabdpt_classifier_colab.ipynb\n"
