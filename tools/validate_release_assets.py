"""Static release-asset validation for the TabDPT classifier DIMER pipeline.
...
PACKAGE_DIR = "src/tabdpt_classifier_pipeline"  # the template's package_dir (default src/<package>)
REPO_NAME = "tabdpt-classifier-pipeline"
EXPECTED_MODEL_ID = "Layer6/TabDPT"
...
MODEL_LOAD_EXPR = f"{PIPELINE_CLASS}.from_pretrained(weights_dir=WEIGHTS_DIR, compile_model=False, use_flash=False, seed=42)"
# Additional 40-hex revisions a document may legitimately cite: the upstream TabDPT-inference code commit.
KNOWN_SHAS: frozenset[str] = frozenset(("9cfb05e0a6bc380ae6c99c08adc8d50dacd4f246",))
# Documents that must name the model id and the immutable revision (this repository keeps weight
# provenance in weights/README.md rather than docs/WEIGHTS.md).
IDENTITY_DOCS = ("README.md", "MODEL_CARD.md", "weights/README.md")
...
NOTEBOOKS = {
    "tabdpt_classifier_colab.ipynb": {
        "template": "notebook_template",
        "profile": "E2E",
        "byod_gates": ("USE_BYOD",),
        "expected_outputs": (
            "outputs/tabdpt_classifier_input_manifest.json",
            "outputs/tabdpt_classifier_evaluation_report.json",
            "outputs/tabdpt_classifier_result.json",
            "outputs/tabdpt_classifier_predictions.csv",
        ),
        "code_markers": (
            "input_manifest = validate_inputs(frame, target_column=TARGET, names=[data_name])",
            "validate_inputs(probe, target_column=TARGET)",
            "print({'ceilings': {'MIN_CLASSES': MIN_CLASSES, 'DECISION_RULE': DECISION_RULE}",
            ...
            "report = evaluation_report(metrics, baseline=baseline, n_holdout=len(test), class_labels=pipe.class_labels_, sample_kind=sample_kind",
            ...
            "assert reloaded.preprocessing_restored_ is True",
            ...
            "importlib.metadata.version('tabdpt')",
            "'use_flash': False",
            "from sklearn.datasets import load_breast_cancer",
        ),
        "markdown_markers": (
            "**Capability:** supervised tabular classification by in-context conditioning",
            "it does **not** gradient-train or fine-tune the pretrained weights",
            "**not calibrated probabilities**",
            "the pipeline ships no acceptance threshold",
            "**in the fitted (sorted) class order**",
            "`majority_class_baseline` is the trivial baseline",
            "**not gradient training**",
            "the verdict is `not-measurable`",
            "`sample-sanity`",
            "**no-preprocessing-refit**",
            "regression, gradient fine-tuning, calibrated probabilities",
        ),
    },
    "tabdpt_classifier_artifact_inference_colab.ipynb": {
        "template": "notebook_template_artifact_inference",
        "profile": "ARTIFACT-INFERENCE",
        "byod_gates": (),
        "expected_outputs": (
            "outputs/tabdpt_classifier_artifact_inference_input_manifest.json",
            "outputs/tabdpt_classifier_artifact_inference_evaluation_report.json",
            "outputs/tabdpt_classifier_artifact_inference_result.json",
            "outputs/tabdpt_classifier_artifact_inference_predictions.csv",
        ),
        "code_markers": (
            "ARTIFACT_DIR = ''",
            "NEW_DATA_PATH = ''",
        ...
        "markdown_markers": (
            "**Capability:** serving-state reconstruction from an externally produced DIMER artifact",
            "produced **outside this execution**",
            "**no artifact is created here**",
            "**Trust boundary.**",
            "legacy compatibility",
            "**no network fallback**",
            "**in the artifact's stored (sorted) class order**",
            "the pipeline ships no probability threshold",
            "its verdict is `not-measurable`",
            "artifact creation, in-notebook support fitting, regression, gradient fine-tuning",
        ),
        "forbidden_code": (
            "load_breast_cancer(",
