...
SUPPORTED_HYPERPARAMETER_KEYS = frozenset(
    {"fine_tune", "n_ensembles", "context_size", "batch_size", "temperature", "seed"}
)
...
class DimerRuntimeConfig:
    ...
    def from_payloads(
        ...
        return cls(
            ...
            temperature=_number(hp, "temperature", 1.0, 0.05, 5.0),
...
def _cap_support_rows(frame: pd.DataFrame, config: DimerRuntimeConfig) -> pd.DataFrame:
    ...
    try:
        ...
        # Preserve every class even when the discarded remainder is too small for sklearn stratification.
...
def prepare_dimer_frames(
    ...
    if config.target_column not in train.columns:
        raise ValueError(f"train.csv is missing target column {config.target_column!r}")
    if train[config.target_column].isna().any():
        raise ValueError("Training target contains missing values")
    counts = train[config.target_column].value_counts()
    if len(counts) < 2:
        raise ValueError("Classification requires at least two target classes")
    if val is None:
        if int(counts.min()) < 2:
            raise ValueError("Every class needs at least two rows when val.csv is absent")
        try:
            train, val = train_test_split(
                train,
                test_size=config.validation_split,
                random_state=config.seed,
                stratify=train[config.target_column],
            )
        except ValueError as exc:
            raise ValueError(f"Unable to create stratified validation split: {exc}") from exc
    ...
    try:
        ...
        _write_json(
            result_path,
            {
                "contractVersion": 1,
                "successful": False,
                "metadata": {"taskType": "tabular_classification", "runId": os.getenv("DIMER_RUN_ID", "")},
