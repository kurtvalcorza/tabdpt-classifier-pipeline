...
def check_pipeline_use_flash(tree: ast.AST, filename: str, cell_idx: int) -> bool:
    """Check if TabDPTClassificationPipeline is called, and verify use_flash=False is passed."""
    found_call = False
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            ...
            if func_name == "TabDPTClassificationPipeline":
                found_call = True
...
def validate_notebook(nb_path: Path) -> None:
    ...
    if not pipeline_instantiated:
        raise AssertionError(f"{nb_path.name}: TabDPTClassificationPipeline was never instantiated.")
