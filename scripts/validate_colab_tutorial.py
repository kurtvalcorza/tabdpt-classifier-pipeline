...
def check_pipeline_use_flash(tree: ast.AST, filename: str, cell_idx: int) -> bool:
    ...
    for node in ast.walk(tree):
        ...
        if name in {"TabDPTClassificationPipeline", "from_pretrained", "load_verified_artifact"}:
            found_call = True
...
def _validate_profile_contract(nb_path: Path, nb: dict, source_text: str) -> None:
    ...
    common = {
        ...
        "decision-rule semantics": "argmax",
        "probability semantics": "uncalibrated",
    }
    ...
    if profile == "E2E":
        required = {
            "BYOD path": "USE_BYOD",
            "meaningful classification baseline": "majority-class baseline",
            ...
            "reload label identity": "labels must match exactly",
            "discrete correctness metric": "accuracy",
            "probability metric": "log_loss",
            "ranking metric": "roc_auc",
            "class coverage check": "Holdout classes absent from support data",
            "class order visibility": "classLabels",
    ...
    if profile == "ARTIFACT-INFERENCE":
        required = {
            ...
            "no-refit reload assertion": "preprocessing_restored_",
            "class-label reconstruction check": "class_labels_ != class_labels",
            "new-data inference": ".predict(",
            "class probabilities": ".predict_proba(",
            "machine-readable predictions": "to_csv(",
        ...
        _require_markers(nb_path, profile, source_text, required)
        forbidden = {
            "self-created sample dataset": "load_breast_cancer(",
