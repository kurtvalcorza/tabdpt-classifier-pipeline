...
from tabdpt_classifier_pipeline import TabDPTClassificationPipeline, TabularFeatureEncoder
...
def test_pipeline_exports_reloadable_preprocessing_state():
    ...
    assert state["classLabels"] == ["no", "yes"]
...
def test_evaluate_rejects_unseen_target_class_before_model_execution():
    pipeline = TabDPTClassificationPipeline()
    pipeline.estimator = object()
    pipeline.target_column = "target"
    pipeline.class_labels_ = ["a", "b"]
    with pytest.raises(ValueError, match="unseen target classes"):
        pipeline.evaluate(pd.DataFrame({"x": [1], "target": ["c"]}))
