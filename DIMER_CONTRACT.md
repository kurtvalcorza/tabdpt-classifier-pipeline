...
The wrapper consumes a labelled pandas table. Categorical encoders and target mappings are fitted only on training data. Numeric missing-value imputation remains inside TabDPT and is fitted by the upstream estimator on the training context. Feature schema is locked after fit.

For a single uploaded labelled CSV, DIMER may create a stratified random validation split only when IID rows are a defensible assumption. Temporal/grouped/entity data must be split upstream and supplied with preserved partitions.

### Inference controls

The versioned manifest exposes `n_ensembles`, `context_size`, `batch_size`, `seed`, and classifier `temperature`. These control throughput/memory and prediction ensembling; they do not train model weights.
...
6. Run a production acceptance test: upload/preserved split → fit context → predict + predict_proba → persist state → fresh reload → prediction parity.
