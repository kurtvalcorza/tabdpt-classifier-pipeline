...
DIMER may additionally supply `model_id` inside `DIMER_HYPERPARAMETERS_JSON` as a **transport-only** Base Model selector. It is intentionally absent from `dimer-pipeline.json`, is accepted without changing TabDPT runtime controls, and all other undeclared hyperparameter keys are rejected.
...
ZIP member paths are normalized and absolute/traversal paths plus duplicate normalized paths are rejected. Direct-directory inputs also enforce file-count, per-file, and total-byte limits before parsing.
...
- `artifacts/artifact.json` containing task, model identity, class names, runtime controls, and context digest.
...
Repository CI proves manifest/runtime mapping, transport-only `model_id` compatibility, dataset safety limits, deterministic split/cap behavior, preprocessing/schema semantics, license/provenance presence, and static tutorial validity. Final acceptance still requires a real pinned checkpoint on GPU and an on-platform execution/deployment test.
