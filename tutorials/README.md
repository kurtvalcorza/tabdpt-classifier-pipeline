...
These notebooks exercise the repository's supported TabDPT classification API and DIMER serving-artifact contract. Static validation is enforced in CI. A notebook is promoted from release candidate to release-grade only after clean-runtime execution evidence is recorded for the exact release revision.

## Notebook registry

| Notebook | Profile | Capability | Default runtime | Release status |
|---|---|---|---|---|
| [`tabdpt_classifier_colab.ipynb`](tabdpt_classifier_colab.ipynb) [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/kurtvalcorza/tabdpt-classifier-pipeline/blob/main/tutorials/tabdpt_classifier_colab.ipynb) | `E2E` | Support-context conditioning with class-coverage checks, stratified leakage-aware evaluation (accuracy / log loss / ROC-AUC) against a majority-class baseline, explicit `argmax` decision rule with uncalibrated class probabilities, preprocessing/capacity reporting, new-data inference with per-class probabilities, DIMER artifact export, no-refit fresh reload with label identity and probability equivalence | Python 3.11+; GPU recommended; CPU supported; `use_flash=False` | **release candidate** — clean-runtime execution evidence required |
| [`tabdpt_classifier_artifact_inference_colab.ipynb`](tabdpt_classifier_artifact_inference_colab.ipynb) [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/kurtvalcorza/tabdpt-classifier-pipeline/blob/main/tutorials/tabdpt_classifier_artifact_inference_colab.ipynb) | `ARTIFACT-INFERENCE` | Validate externally supplied artifact/model provenance and class-label mapping, restore fitted preprocessing/support state without refit, validate/score new data with per-class probabilities, report capacity/context, export predictions/provenance | Python 3.11+; GPU recommended; CPU supported; `use_flash=False` | **release candidate** — external-artifact clean-runtime evidence required |
...
The E2E notebook reports input shape/missingness, class counts, model feature ceiling, whether feature reduction activates, requested context size, and the effective upper bound on support rows. The artifact-inference notebook reports the same serving-relevant capacity information for an externally supplied artifact and validates new numeric data so non-numeric or infinite values do not silently become imputed values.

## Artifact contract and trust boundary

The reusable DIMER serving state includes the labelled support table, the fitted class-label mapping (`classLabels`, sorted order defines the probability column order), fitted repository encoder state, fitted upstream imputer/scaler state, any fitted PCA basis needed for wide tables, and the exact pinned base-model contract.

`export_artifact_bundle()` emits an explicit v3 release artifact. `validate_artifact_bundle()` checks artifact format/task, class-label mapping, fitted preprocessing consistency, support-table path/size/SHA-256, and the declared base-model repository/revision/filename/digest/upstream commit before model loading. `load_verified_artifact()` reconstructs new release artifacts from saved fitted preprocessing state **without fitting those preprocessing statistics again**.
...
CI covers notebook JSON/source structure, Python-cell compilation, profile metadata, runtime-floor markers, decision-rule/probability-semantics markers, preprocessing/capacity workflow assertions, artifact-contract/no-refit/class-mapping regression tests, and ordinary repository tests. These checks are not evidence that the current Colab model host/runtime/GPU path executes successfully.

Before marking either notebook `release-grade`, record a clean execution against the exact candidate commit. For the `E2E` notebook, verify the default sample path, pinned-weight digest, preprocessing/capacity report, class-coverage check, metrics/baseline, machine-readable exports with `proba_<class>` columns, artifact export, no-refit fresh reload, and the label-identity/probability-equivalence assertions. For `ARTIFACT-INFERENCE`, use an artifact produced in a separate execution/session plus separately supplied new data, and verify pre-load validation, `preprocessing_restored_ = True`, reconstruction, capacity/context reporting, prediction, and exported results/provenance.

Until clean-runtime execution evidence is recorded for the exact candidate commit, the correct status is **release candidate**, not release-grade. Execution records are appended below.

### Execution records

| Date (UTC) | Notebook revision | Environment | Engine | Outcome |
|---|---|---|---|---|
| 2026-09-11 | `45071534d021` (clean tree) | Local host `Kurt-Valcorza`, Windows-11-10.0.26200-SP0; Python 3.12.10; torch 2.7.1+cu128; tabdpt 1.2.0; numpy 2.3.0 (exact `requirements-colab.txt` lock set); GPU NVIDIA GeForce RTX 5070 Ti Laptop GPU; `use_flash=False` | `scripts/execute_notebook_release.py --skip-bootstrap`: nbclient, one fresh IPython kernel per notebook; `/content` → `D:\tabdpt-run-lock\content`; `google.colab.files.upload()` served from explicit paths | **PASS** — E2E: sample accuracy 0.9912 / log_loss 0.0594 / roc_auc 0.9977 vs majority baseline 0.6316; artifact exported; no-refit fresh reload with identical labels and equivalent probabilities. ARTIFACT-INFERENCE (second kernel, artifact + 8 fresh rows supplied externally): `preprocessing_restored_ = True`, 8 rows scored with `proba_benign`/`proba_malignant`. Evidence JSON: `release-evidence.json` from that run. **Not a Google Colab run** — a Colab record is still required before either notebook is marked release-grade. |
